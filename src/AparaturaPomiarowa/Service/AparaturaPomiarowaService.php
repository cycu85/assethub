...
class AparaturaPomiarowaService
{
    ...
    public function createEquipment(array $data, User $user): AparaturaPomiarowaEquipment
    {
        ...
        if ($this->equipmentRepository->findByInventoryNumber($equipment->getInventoryNumber())) {
            throw new BusinessLogicException('Miernik o takim numerze inwentarzowym już istnieje');
        }
        ...
        $this->logger->info('Utworzono miernik aparaturapomiarowy', [
    ...
    public function updateEquipment(AparaturaPomiarowaEquipment $equipment, array $data, User $user): AparaturaPomiarowaEquipment
    {
        ...
        $this->logger->info('Zaktualizowano miernik aparaturapomiarowy', [
            'equipment_id' => $equipment->getId(),
            'user' => $user->getUsername()
        ]);
    ...
    public function deleteEquipment(AparaturaPomiarowaEquipment $equipment, User $user): void
    {
        if ($equipment->isAssigned()) {
            throw new BusinessLogicException('Nie można usunąć przypisanego mierniku');
        }

        if (!$equipment->getEquipmentSets()->isEmpty()) {
            throw new BusinessLogicException('Nie można usunąć mierniku będącego częścią zestawu');
        }

        if (!$equipment->getReviews()->isEmpty()) {
            throw new BusinessLogicException('Nie można usunąć mierniku z historią kalibracjaów');
        }
        ...
        $this->logger->info('Usunięto miernik aparaturapomiarowy', [
    ...
    public function assignEquipment(AparaturaPomiarowaEquipment $equipment, User $assignee, User $assignedBy, ?string $notes = null): AparaturaPomiarowaEquipment
    {
        if (!$equipment->isAvailable()) {
            throw new BusinessLogicException('Miernik nie jest dostępny do przypisania');
        }
        ...
        $this->logger->info('Przypisano miernik aparaturapomiarowy', [
            'equipment_id' => $equipment->getId(),
            'assigned_to' => $assignee->getUsername(),
    ...
    {
        if (!$equipment->isAssigned()) {
            throw new BusinessLogicException('Miernik nie jest przypisany');
        }
        ...
        $this->logger->info('Cofnięto przypisanie mierniku aparaturapomiarowego', [
            'equipment_id' => $equipment->getId(),
            'previous_assignee' => $previousAssignee?->getUsername(),
    ...
    public function createEquipmentSet(array $data, User $user): AparaturaPomiarowaEquipmentSet
    {
        ...
        $this->logger->info('Utworzono zestaw aparaturapomiarowy', [
    ...
    public function updateEquipmentSet(AparaturaPomiarowaEquipmentSet $equipmentSet, array $data, User $user): AparaturaPomiarowaEquipmentSet
    {
        ...
        $this->logger->info('Zaktualizowano zestaw aparaturapomiarowy', [
            'set_id' => $equipmentSet->getId(),
            'user' => $user->getUsername()
        ]);
    ...
    public function deleteEquipmentSet(AparaturaPomiarowaEquipmentSet $equipmentSet, User $user): void
    {
        ...
        if (!$equipmentSet->getReviews()->isEmpty()) {
            throw new BusinessLogicException('Nie można usunąć zestawu z historią kalibracjaów');
        }
        ...
        $this->logger->info('Usunięto zestaw aparaturapomiarowy', [
            'set_id' => $setId,
            'user' => $user->getUsername()
        ]);
    }
    ...
    public function addEquipmentToSet(AparaturaPomiarowaEquipmentSet $equipmentSet, AparaturaPomiarowaEquipment $equipment, User $user): AparaturaPomiarowaEquipmentSet
    {
        if ($equipmentSet->getEquipment()->contains($equipment)) {
            throw new BusinessLogicException('Miernik już należy do tego zestawu');
        }
    ...
    {
        if (!$equipmentSet->getEquipment()->contains($equipment)) {
            throw new BusinessLogicException('Miernik nie należy do tego zestawu');
        }
    ...
    {
        if (empty($data['name'])) {
            throw new ValidationException('Nazwa mierniku jest wymagana');
        }
        ...
        if (empty($data['equipment_type'])) {
            throw new ValidationException('Typ mierniku jest wymagany');
        }
    }
    ...
    public function deleteReview(AparaturaPomiarowaReview $review, User $user): void
    {
        if ($review->getStatus() === AparaturaPomiarowaReview::STATUS_COMPLETED) {
            throw new BusinessLogicException('Nie można usunąć zakończonego kalibracjau');
        }
    ...
    public function sendReview(AparaturaPomiarowaReview $review, User $user): AparaturaPomiarowaReview
    {
        if ($review->getStatus() !== AparaturaPomiarowaReview::STATUS_PREPARATION) {
            throw new BusinessLogicException('Kalibracja można wysłać tylko ze statusu "Przygotowanie"');
        }
        ...
        // Zmiana statusu mierniku lub zestawu na "w trakcie kalibracjau"
        if ($review->getEquipment()) {
            // Kalibracja pojedynczego mierniku
    ...
    public function completeReview(AparaturaPomiarowaReview $review, array $data, User $user): AparaturaPomiarowaReview
    {
        if ($review->getStatus() !== AparaturaPomiarowaReview::STATUS_SENT) {
            throw new BusinessLogicException('Kalibracja można zakończyć tylko ze statusu "Wysłane"');
        }
        ...
        // Aktualizacja wyników kalibracjau
        ...
        if (isset($data['next_review_date'])) {
            ...
            // Aktualizacja daty następnego kalibracjau w sprzęcie/zestawie
            if ($review->getEquipment()) {
            ...
            if ($review->getEquipmentSet()) {
                ...
                // Aktualizacja dat następnego kalibracjau dla wszystkich elementów zestawu
        ...
        // Przywrócenie statusu mierniku lub zestawu po zakończeniu kalibracjau
        if ($review->getEquipment()) {
            // Kalibracja pojedynczego mierniku - przywrócenie statusu na podstawie przypisania
            $equipment = $review->getEquipment();
    ...
    private function calculateNextReviewDateForEquipment(AparaturaPomiarowaEquipment $equipment, \DateTimeInterface $completedDate, \DateTimeInterface $fallbackDate): \DateTimeInterface
    {
        ...
        // Jeśli element ma ustawiony okres kalibracjau, oblicz datę na podstawie daty zakończenia + okres
        ...
        // Jeśli element nie ma ustawionego okresu kalibracjau, użyj daty z zestawu
        return $fallbackDate;
    }
}