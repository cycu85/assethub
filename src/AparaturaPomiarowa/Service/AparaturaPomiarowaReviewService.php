...
use App\AparaturaPomiarowa\Repository\AparaturaPomiarowaReviewEquipmentRepository;
use App\Entity\User;
...
class AparaturaPomiarowaReviewService
{
    public function __construct(
        ...
        private AparaturaPomiarowaReviewEquipmentRepository $reviewEquipmentRepository,
    ...
    public function createEquipmentReview(AparaturaPomiarowaEquipment $equipment, array $data, User $user): AparaturaPomiarowaReview
    {
        ...
        $this->auditService->logCrudOperation($user, 'AparaturaPomiarowaReview', $review->getId(), 'CREATE', array_merge($data, [
            'type' => 'equipment_review',
            'equipment_id' => $equipment->getId()
        ]));

        $this->logger->info('Utworzono kalibrację urządzenia', [
    ...
    public function createEquipmentSetReview(AparaturaPomiarowaEquipmentSet $equipmentSet, array $data, User $user, array $selectedEquipmentIds = []): AparaturaPomiarowaReview
    {
        $this->validateReviewData($data);
        ...
        $this->logger->info('Utworzono kalibrację zestawu', [
            'review_id' => $review->getId(),
            'equipment_set_id' => $equipmentSet->getId(),
        ...
        // Powiadomienie email będzie wysłane automatycznie przez AparaturaPomiarowaEventSubscriber
    ...
    public function sendToReview(AparaturaPomiarowaReview $review, User $user): AparaturaPomiarowaReview
    {
        if (!$review->canBeSent()) {
            throw new BusinessLogicException('Kalibracja nie może być wysłana w aktualnym stanie: ' . $review->getStatusDisplayName());
        }
        ...
        $this->logger->info('Wysłano kalibrację', [
            'review_id' => $review->getId(),
            'review_number' => $review->getReviewNumber(),
    ...
    public function completeReview(AparaturaPomiarowaReview $review, array $data, User $user): AparaturaPomiarowaReview
    {
        if (!$review->canBeCompleted()) {
            throw new BusinessLogicException('Kalibracja nie może być zakończona w aktualnym stanie: ' . $review->getStatusDisplayName());
        }
        ...
        $this->logger->info('Zakończono kalibrację', [
            'review_id' => $review->getId(),
            'review_number' => $review->getReviewNumber(),
            'result' => $review->getResult(),
    ...
    public function cancelReview(AparaturaPomiarowaReview $review, User $user, ?string $reason = null): AparaturaPomiarowaReview
    {
        if (!$review->canBeCancelled()) {
            throw new BusinessLogicException('Kalibracja nie może być anulowana w aktualnym stanie: ' . $review->getStatusDisplayName());
        }
        ...
        $this->logger->info('Anulowano kalibrację', [
            'review_id' => $review->getId(),
            'review_number' => $review->getReviewNumber(),
            'reason' => $reason,
    ...
    private function validateReviewData(array $data): void
    {
        if (empty($data['planned_date'])) {
            throw new ValidationException('Data planowanej kalibracji jest wymagana');
        }

        if (empty($data['review_type'])) {
            throw new ValidationException('Typ kalibracji jest wymagany');
        }
    }
    ...
    private function validateCompletionData(array $data): void
    {
        if (empty($data['result'])) {
            throw new ValidationException('Wynik kalibracji jest wymagany');
        }

        if (!in_array($data['result'], array_keys(AparaturaPomiarowaReview::RESULTS))) {
            throw new ValidationException('Nieprawidłowy wynik kalibracji');
        }
    }

    private function checkActiveReviewsForEquipment(AparaturaPomiarowaEquipment $equipment): void
    {
        $activeReviews = $this->reviewRepository->getActiveReviewsForEquipment($equipment);
        if (!empty($activeReviews)) {
            throw new BusinessLogicException('Urządzenie ma już aktywną kalibrację');
        }
    }

    private function checkActiveReviewsForEquipmentSet(AparaturaPomiarowaEquipmentSet $equipmentSet): void
    {
        $activeReviews = $this->reviewRepository->getActiveReviewsForEquipmentSet($equipmentSet);
        if (!empty($activeReviews)) {
            throw new BusinessLogicException('Zestaw ma już aktywną kalibrację');
        }
    }
    ...
    private function validateSelectedEquipment(AparaturaPomiarowaEquipmentSet $equipmentSet, array $selectedEquipmentIds): void
    {
        ...
        foreach ($selectedEquipmentIds as $equipmentId) {
            if (!in_array($equipmentId, $setEquipmentIds)) {
                throw new BusinessLogicException('Wybrane urządzenie nie należy do zestawu');
            }
        }
    }

    private function populateReviewFromArray(AparaturaPomiarowaReview $review, array $data): void
    {
    ...
    private function updateSubjectAfterReview(AparaturaPomiarowaReview $review, array $data, User $user): void
    {
        ...
        if ($review->isForSingleEquipment()) {
            ...
            $newStatus = match ($review->getResult()) {
                AparaturaPomiarowaReview::RESULT_PASSED, 
                AparaturaPomiarowaReview::RESULT_CONDITIONALLY_PASSED => $equipment->isAssigned() 
                    ? AparaturaPomiarowaEquipment::STATUS_ASSIGNED 
                    : AparaturaPomiarowaEquipment::STATUS_AVAILABLE,
                AparaturaPomiarowaReview::RESULT_FAILED => AparaturaPomiarowaEquipment::STATUS_DAMAGED,
                default => AparaturaPomiarowaEquipment::STATUS_AVAILABLE
            };
            ...
            $newStatus = match ($review->getResult()) {
                AparaturaPomiarowaReview::RESULT_PASSED, 
                AparaturaPomiarowaReview::RESULT_CONDITIONALLY_PASSED => $equipmentSet->isAssigned() 
                    ? AparaturaPomiarowaEquipmentSet::STATUS_ASSIGNED 
                    : AparaturaPomiarowaEquipmentSet::STATUS_AVAILABLE,
                AparaturaPomiarowaReview::RESULT_FAILED => AparaturaPomiarowaEquipmentSet::STATUS_INCOMPLETE,
                default => AparaturaPomiarowaEquipmentSet::STATUS_AVAILABLE
            };
    ...
    private function restoreSubjectStatusAfterCancel(AparaturaPomiarowaReview $review, User $user): void
    {
        if ($review->isForSingleEquipment()) {
            $equipment = $review->getEquipment();
     ...
     */
    private function updateEquipmentReviewResults(AparaturaPomiarowaReview $review, array $data): void
    {
        ...
        foreach ($review->getReviewEquipments() as $reviewEquipment) {
            ...
            if (!$reviewEquipment->getIndividualResult()) {
                $reviewEquipment->setIndividualResult(AparaturaPomiarowaReviewEquipment::RESULT_INHERITED);
            }
     ...
     */
    public function getEquipmentReviewHistory(AparaturaPomiarowaEquipment $equipment): array
    {
     ...
     */
    public function getReviewEquipments(AparaturaPomiarowaReview $review): array
    {
     ...
     */
    public function getEquipmentHistoryWithPagination(AparaturaPomiarowaEquipment $equipment, int $page = 1, int $limit = 25): array
    {
    ...
    /**
     * Generate unique review number in format CAL/YYYY/MM/XXX
     */
    private function generateReviewNumber(): string
    {
        ...
        $lastReview = $this->reviewRepository->createQueryBuilder('r')
            ...
            ->setParameter('pattern', "CAL/{$year}/{$month}/%")
        ...
        return sprintf('CAL/%s/%s/%03d', $year, $month, $nextNumber);
    }
    ...
    public function updateEquipmentReviewResult(
        AparaturaPomiarowaReview $review, 
        AparaturaPomiarowaEquipment $equipment, 
        string $result, 
     ...
     */
    public function wasEquipmentInSetReview(AparaturaPomiarowaEquipment $equipment): bool
    {
     ...
     */
    public function getLastReviewEquipmentForEquipment(AparaturaPomiarowaEquipment $equipment): ?AparaturaPomiarowaReviewEquipment
    {
        $history = $this->getEquipmentReviewHistory($equipment);
        
        $completedReviews = array_filter($history, function(AparaturaPomiarowaReviewEquipment $reviewEquipment) {
    ...
    public function addEquipmentToReview(AparaturaPomiarowaReview $review, AparaturaPomiarowaEquipment $equipment, User $user): AparaturaPomiarowaReviewEquipment
    {
        // Sprawdzenie czy urządzenie już nie jest w kalibracji
        foreach ($review->getReviewEquipments() as $reviewEquipment) {
            if ($reviewEquipment->getEquipment() && $reviewEquipment->getEquipment()->getId() === $equipment->getId()) {
                throw new BusinessLogicException(sprintf('Urządzenie "%s" już znajduje się w tej kalibracji.', $equipment->getName()));
            }
        }

        // Sprawdzenie czy kalibracja nie jest zakończona
        if ($review->getStatus() === 'completed') {
            throw new BusinessLogicException('Nie można dodawać urządzenia do zakończonej kalibracji.');
        }

        // Tworzenie nowego AparaturaPomiarowaReviewEquipment
        $reviewEquipment = new AparaturaPomiarowaReviewEquipment();
        $reviewEquipment->setReview($review);
        $reviewEquipment->setEquipment($equipment);
        
        // Zapisanie aktualnych danych urządzenia na moment kalibracji
    ...
    public function removeEquipmentFromReview(AparaturaPomiarowaReview $review, AparaturaPomiarowaReviewEquipment $reviewEquipment, User $user): void
    {
        // Sprawdzenie czy kalibracja nie jest zakończona
        if ($review->getStatus() === 'completed') {
            throw new BusinessLogicException('Nie można usuwać urządzenia z zakończonej kalibracji.');
        }

        // Sprawdzenie czy AparaturaPomiarowaReviewEquipment należy do tej kalibracji
        if ($reviewEquipment->getReview()->getId() !== $review->getId()) {
            throw new BusinessLogicException('Element urządzenia nie należy do tej kalibracji.');
        }
     ...
     */
    private function updateSubjectStatusForReview(AparaturaPomiarowaReview $review, User $user): void
    {
        if ($review->isForSingleEquipment()) {
            $equipment = $review->getEquipment();
            ...
            foreach ($review->getReviewEquipments() as $reviewEquipment) {
                if ($reviewEquipment->getEquipment()) {
                    $reviewEquipment->getEquipment()->setStatus(AparaturaPomiarowaEquipment::STATUS_IN_REVIEW);
