...
class AparaturaPomiarowaReviewService
{
    ...
    public function createEquipmentReview(AparaturaPomiarowaEquipment $equipment, array $data, User $user): AparaturaPomiarowaReview
    {
        ...
        $this->logger->info('Utworzono kalibracja mierniku', [
    ...
    public function createEquipmentSetReview(AparaturaPomiarowaEquipmentSet $equipmentSet, array $data, User $user, array $selectedEquipmentIds = []): AparaturaPomiarowaReview
    {
        ...
        $this->logger->info('Utworzono kalibracja zestawu', [
            'review_id' => $review->getId(),
            'equipment_set_id' => $equipmentSet->getId(),
    ...
    {
        if (!$review->canBeSent()) {
            throw new BusinessLogicException('Kalibracja nie może być wysłany w aktualnym stanie: ' . $review->getStatusDisplayName());
        }
        ...
        $this->logger->info('Wysłano kalibracja', [
            'review_id' => $review->getId(),
            'review_number' => $review->getReviewNumber(),
    ...
    public function completeReview(AparaturaPomiarowaReview $review, array $data, User $user): AparaturaPomiarowaReview
    {
        if (!$review->canBeCompleted()) {
            throw new BusinessLogicException('Kalibracja nie może być zakończony w aktualnym stanie: ' . $review->getStatusDisplayName());
        }
        ...
        $this->logger->info('Zakończono kalibracja', [
            'review_id' => $review->getId(),
            'review_number' => $review->getReviewNumber(),
            'result' => $review->getResult(),
    ...
    public function cancelReview(AparaturaPomiarowaReview $review, User $user, ?string $reason = null): AparaturaPomiarowaReview
    {
        if (!$review->canBeCancelled()) {
            throw new BusinessLogicException('Kalibracja nie może być anulowany w aktualnym stanie: ' . $review->getStatusDisplayName());
        }
        ...
        $this->logger->info('Anulowano kalibracja', [
            'review_id' => $review->getId(),
            'review_number' => $review->getReviewNumber(),
            'reason' => $reason,
    ...
    private function validateReviewData(array $data): void
    {
        if (empty($data['planned_date'])) {
            throw new ValidationException('Data planowanego kalibracjau jest wymagana');
        }

        if (empty($data['review_type'])) {
            throw new ValidationException('Typ kalibracjau jest wymagany');
        }
    }
    ...
    private function validateCompletionData(array $data): void
    {
        if (empty($data['result'])) {
            throw new ValidationException('Wynik kalibracjau jest wymagany');
        }

        if (!in_array($data['result'], array_keys(AparaturaPomiarowaReview::RESULTS))) {
            throw new ValidationException('Nieprawidłowy wynik kalibracjau');
        }
    }
    ...
    private function checkActiveReviewsForEquipment(AparaturaPomiarowaEquipment $equipment): void
    {
        ...
        if (!empty($activeReviews)) {
            throw new BusinessLogicException('Miernik ma już aktywny kalibracja');
        }
    }
    ...
    private function checkActiveReviewsForEquipmentSet(AparaturaPomiarowaEquipmentSet $equipmentSet): void
    {
        ...
        if (!empty($activeReviews)) {
            throw new BusinessLogicException('Zestaw ma już aktywny kalibracja');
        }
    }
    ...
    private function validateSelectedEquipment(AparaturaPomiarowaEquipmentSet $equipmentSet, array $selectedEquipmentIds): void
    {
        ...
        foreach ($selectedEquipmentIds as $equipmentId) {
            if (!in_array($equipmentId, $setEquipmentIds)) {
                throw new BusinessLogicException('Wybrany miernik nie należy do zestawu');
            }
        }
    }
    ...
    {
        // Sprawdzenie czy miernik już nie jest w kalibracjazie
        foreach ($review->getReviewEquipments() as $reviewEquipment) {
            if ($reviewEquipment->getEquipment() && $reviewEquipment->getEquipment()->getId() === $equipment->getId()) {
                throw new BusinessLogicException(sprintf('Miernik "%s" już znajduje się w tym kalibracjazie.', $equipment->getName()));
            }
        }

        // Sprawdzenie czy kalibracja nie jest zakończony
        if ($review->getStatus() === 'completed') {
            throw new BusinessLogicException('Nie można dodawać mierniku do zakończonego kalibracjau.');
        }
        ...
        // Zapisanie aktualnych danych mierniku na moment kalibracjau
    ...
    public function removeEquipmentFromReview(AparaturaPomiarowaReview $review, AparaturaPomiarowaReviewEquipment $reviewEquipment, User $user): void
    {
        // Sprawdzenie czy kalibracja nie jest zakończony
        if ($review->getStatus() === 'completed') {
            throw new BusinessLogicException('Nie można usuwać mierniku z zakończonego kalibracjau.');
        }

        // Sprawdzenie czy AparaturaPomiarowaReviewEquipment należy do tego kalibracjau
        if ($reviewEquipment->getReview()->getId() !== $review->getId()) {
            throw new BusinessLogicException('Element mierniku nie należy do tego kalibracjau.');
        }
