...
class AparaturaPomiarowaEquipmentSetController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        $equipmentSetsPagination = $this->aparaturaPomiarowaService->getEquipmentSetsWithPagination($page, 1000, $filters);
        $statistics = $this->aparaturaPomiarowaService->getEquipmentSetStatistics();
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                $equipmentSet = $this->aparaturaPomiarowaService->createEquipmentSet($data, $user);
                
                $this->addFlash('success', 'Zestaw aparaturaPomiarowa został utworzony pomyślnie.');
                ...
                $this->logger->error('Failed to create aparaturaPomiarowa equipment set', [
                    'error' => $e->getMessage(),
    ...
    public function show(AparaturaPomiarowaEquipmentSet $equipmentSet, Request $request): Response
    {
        ...
        $canEdit = $this->authorizationService->hasPermission($user, 'aparatura_pomiarowa', 'EDIT') 
                   || ($equipmentSet->getAssignedTo() === $user && $this->authorizationService->hasPermission($user, 'aparatura_pomiarowa', 'VIEW'));
        $canDelete = $this->authorizationService->hasPermission($user, 'aparatura_pomiarowa', 'DELETE');
        $canAssign = $this->authorizationService->hasPermission($user, 'aparatura_pomiarowa', 'ASSIGN');
        $canReview = $this->authorizationService->hasPermission($user, 'aparatura_pomiarowa', 'REVIEW');
        $canTransfer = $this->authorizationService->hasPermission($user, 'aparatura_pomiarowa', 'TRANSFER');
        $canManageEquipment = $this->authorizationService->hasPermission($user, 'aparatura_pomiarowa', 'EDIT');
        ...
        $reviewRepository = $this->entityManager->getRepository(\App\AparaturaPomiarowa\Entity\AparaturaPomiarowaReview::class);
    ...
    public function edit(AparaturaPomiarowaEquipmentSet $equipmentSet, Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                $this->aparaturaPomiarowaService->updateEquipmentSet($equipmentSet, $data, $user);
                
                $this->addFlash('success', 'Zestaw aparaturaPomiarowa został zaktualizowany pomyślnie.');
                return $this->redirectToRoute('aparatura_pomiarowa_equipment_set_show', ['id' => $equipmentSet->getId()]);
                ...
                $this->addFlash('error', 'Wystąpił nieoczekiwany błąd.');
                $this->logger->error('Failed to update aparaturaPomiarowa equipment set', [
                    'equipment_set_id' => $equipmentSet->getId(),
                    'error' => $e->getMessage(),
                    'user' => $user->getUsername()
                ]);
    ...
    public function delete(AparaturaPomiarowaEquipmentSet $equipmentSet, Request $request): Response
    {
        ...
        try {
            ...
            $this->aparaturaPomiarowaService->deleteEquipmentSet($equipmentSet, $user);
            ...
            $this->logger->error('Failed to delete aparaturaPomiarowa equipment set', [
                'equipment_set_id' => $equipmentSet->getId(),
                'error' => $e->getMessage(),
                'user' => $user->getUsername()
            ]);
    ...
    #[Route('/{id}/equipment/add', name: 'aparatura_pomiarowa_equipment_set_add_equipment', requirements: ['id' => '\d+'])]
    public function addEquipment(AparaturaPomiarowaEquipmentSet $equipmentSet, Request $request): Response
    {
                ...
                try {
                    ...
                    foreach ($equipmentIds as $equipmentId) {
                        ...
                        if ($equipment && !$equipmentSet->getEquipment()->contains($equipment)) {
                            $this->aparaturaPomiarowaService->addEquipmentToSet($equipmentSet, $equipment, $user);
                            $addedCount++;
                        }
                    }
        ...
        $availableEquipment = $this->aparaturaPomiarowaService->getAvailableEquipmentForSet();
    ...
    #[Route('/{id}/equipment/{equipmentId}/remove', name: 'aparatura_pomiarowa_equipment_set_remove_equipment', requirements: ['id' => '\d+', 'equipmentId' => '\d+'], methods: ['POST'])]
    public function removeEquipment(AparaturaPomiarowaEquipmentSet $equipmentSet, int $equipmentId, Request $request): Response
    {
        $user = $this->getUser();
        ...
        try {
            ...
            $this->aparaturaPomiarowaService->removeEquipmentFromSet($equipmentSet, $equipment, $user);
    ...
    #[Route('/{id}/equipment/remove-bulk', name: 'aparatura_pomiarowa_equipment_set_remove_bulk_equipment', requirements: ['id' => '\d+'], methods: ['POST'])]
    public function removeBulkEquipment(AparaturaPomiarowaEquipmentSet $equipmentSet, Request $request): Response
    {
        $user = $this->getUser();
        ...
        if (empty($equipmentIds)) {
            ...
            return $this->redirectToRoute('aparatura_pomiarowa_equipment_set_add_equipment', ['id' => $equipmentSet->getId()]);
        }
    ...
    #[Route('/available-equipment', name: 'aparatura_pomiarowa_available_equipment_modal')]
    ...
    #[Route('/{id}/attachment/upload', name: 'aparatura_pomiarowa_equipment_set_attachment_upload', requirements: ['id' => '\d+'], methods: ['POST'])]
    public function uploadAttachment(AparaturaPomiarowaEquipmentSet $equipmentSet, Request $request): Response
    {
        $user = $this->getUser();
        ...
        try {
            ...
            $equipmentSet = $this->entityManager->find(AparaturaPomiarowaEquipmentSet::class, $equipmentSet->getId());
            if (!$equipmentSet) {
            ...
            $uploadedCount = 0;
            $uploadDir = $this->getParameter('kernel.project_dir') . '/var/uploads/aparatura-pomiarowa/equipment-sets/';
    ...
    #[Route('/{id}/attachment/{filename}/download', name: 'aparatura_pomiarowa_equipment_set_attachment_download', requirements: ['id' => '\d+'])]
    public function downloadAttachment(AparaturaPomiarowaEquipmentSet $equipmentSet, string $filename, Request $request): Response
    {
        $user = $this->getUser();
        ...
        $filePath = $this->getParameter('kernel.project_dir') . '/var/uploads/aparatura-pomiarowa/equipment-sets/' . $filename;
    ...
    #[Route('/{id}/attachment/{filename}/delete', name: 'aparatura_pomiarowa_equipment_set_attachment_delete', requirements: ['id' => '\d+'], methods: ['POST'])]
    public function deleteAttachment(AparaturaPomiarowaEquipmentSet $equipmentSet, string $filename, Request $request): Response
    {
        $user = $this->getUser();
    ...
    #[Route('/transfer/{setId}/prepare', name: 'aparatura_pomiarowa_transfer_prepare', requirements: ['setId' => '\d+'], methods: ['POST'])]
    public function prepareTransfer(int $setId, Request $request): Response
    {
        ...
        $equipmentSet = $this->entityManager->find(AparaturaPomiarowaEquipmentSet::class, $setId);
        if (!$equipmentSet) {
    ...
    #[Route('/transfer/{id}/complete', name: 'aparatura_pomiarowa_transfer_complete', requirements: ['id' => '\d+'], methods: ['POST'])]
    public function completeTransfer(int $id, Request $request): Response
    {
        ...
        $transfer = $this->entityManager->find(AparaturaPomiarowaTransfer::class, $id);
        if (!$transfer) {
        ...
        return $this->redirectToRoute('aparatura_pomiarowa_equipment_set_show', ['id' => $transfer->getEquipmentSet()->getId()]);
    }

    #[Route('/transfer/{id}/protocol/download', name: 'aparatura_pomiarowa_transfer_protocol_download', requirements: ['id' => '\d+'])]
    ...
    #[Route('/return/{setId}/prepare', name: 'aparatura_pomiarowa_return_prepare', requirements: ['setId' => '\d+'], methods: ['POST'])]
    ...
    #[Route('/transfer/{id}/return', name: 'aparatura_pomiarowa_transfer_return', requirements: ['id' => '\d+'], methods: ['POST'])]
    ...
    #[Route('/return/{id}/complete', name: 'aparatura_pomiarowa_return_complete', requirements: ['id' => '\d+'], methods: ['POST'])]
    ...
    #[Route('/return/{id}/protocol/download', name: 'aparatura_pomiarowa_return_protocol_download', requirements: ['id' => '\d+'])]
    public function downloadReturnProtocol(int $id, Request $request): Response
    {
        ...
        $filePath = $this->getParameter('kernel.project_dir') . '/var/uploads/aparatura-pomiarowa/transfers/' . $transfer->getReturnProtocolFilename();
    ...
    private function generateReturnProtocolPDF(AparaturaPomiarowaTransfer $transfer): string
    {
    ...
    private function generateTransferProtocolPDF(AparaturaPomiarowaTransfer $transfer): string
    {
        $equipmentSet = $transfer->getEquipmentSet();
        $recipient = $transfer->getRecipient();
        $handedBy = $transfer->getHandedBy();
