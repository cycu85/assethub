    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $builder
            ->add('plannedDate', DateType::class, [
                'label' => 'Planowana data kalibracjau',
                'widget' => 'single_text',
                'attr' => [
                    'class' => 'form-control'
                ],
                'constraints' => [
                    new Assert\NotBlank(['message' => 'Data planowanego kalibracjau jest wymagana'])
                ]
            ])
            
            ->add('reviewType', ChoiceType::class, [
                'label' => 'Typ kalibracjau',
                'choices' => $reviewTypes,
                'placeholder' => 'Wybierz typ kalibracjau',
                'attr' => [
                    'class' => 'form-select'
                ],
                'constraints' => [
                    new Assert\NotBlank(['message' => 'Typ kalibracjau jest wymagany'])
                ]
            ])
            
            ->add('reviewCompany', TextType::class, [
                'label' => 'Firma przeprowadzająca kalibracja',
                'required' => false,
                'attr' => [
                    'class' => 'form-control',
        ...
        // Dodanie pól wyboru mierniku/zestawu tylko jeśli nie są określone w opcjach
        if (!$options['equipment'] && !$options['equipment_set']) {
            $builder
                ->add('equipment', EntityType::class, [
                    ...
                    'label' => 'Miernik do kalibracjau',
                    'required' => false,
                    'placeholder' => 'Wybierz miernik',
                    'attr' => [
                        'class' => 'form-select',
                ...
                ->add('equipmentSet', EntityType::class, [
                    ...
                    'label' => 'Zestaw mierniku do kalibracjau',
                    'required' => false,
        ...
        // Pola do wypełnienia po zakończeniu kalibracjau (tylko w trybie edycji)
        if ($options['mode'] === 'completion') {
            $builder
                ->add('result', ChoiceType::class, [
                    'label' => 'Wynik kalibracjau',
                    'choices' => AparaturaPomiarowaReview::RESULTS,
                    'placeholder' => 'Wybierz wynik kalibracjau',
                    'attr' => [
                        'class' => 'form-select'
                    ],
                    'constraints' => [
                        new Assert\NotBlank(['message' => 'Wynik kalibracjau jest wymagany'])
                    ]
                ])
                ...
                ->add('nextReviewDate', DateType::class, [
                    'label' => 'Data następnego kalibracjau',
                    'required' => false,
                    'widget' => 'single_text',
                    'attr' => [
                        'class' => 'form-control'
                    ],
                    'help' => 'Zalecana data kolejnego kalibracjau'
                ]);
        }
        ...
        if ($options['include_submit']) {
            $submitLabel = match ($options['mode']) {
                'completion' => 'Zakończ kalibracja',
                'edit' => 'Aktualizuj kalibracja',
                default => 'Utwórz kalibracja'
            };
